...
import './TypeFieldMappingTable.scss';
...
import mockData from '../dummydata/pairwise_mock';


class TypeFieldMappingTable extends Component {
  ...
  constructor(props) {
    ...
    const { typesData, pairwiseTypesActions } = this.props;
    console.log("cccc",typesData);
    console.log("cccc:detail",typesData.detail);
    //const typesDataDetail = typesData.detail.map((data) => {
      const typesDataDetail = mockData.detail.map((data) => {
    ...
    pairwiseTypesActions.updateState({ typesDataDetail });

    this.state = {
      treeValue: TreeState.create(typesDataDetail)
    };
    this.state = {
  ...
  render() {
    ...
    return (
      <div className="type-field-mapping-table">
      <Fragment>
        <Row className="my-1">
          <Col>
            <ButtonGroup  >
          ...
          <Col>
            ...
            >
              ...
              <TreeTable.Column basis="350px" grow="0"
                ...
                renderHeaderCell={() => <span>조합값</span>} />
  ...
  renderMagnifier = (row) => {
    ...
    return (
      (row.metadata.hasChildren)
        ? ('')
        :<Button className="ml-4" variant="light" size="sm" onClick={() => {console.log("클릭시", this.props.testResultDetail(row.data.value)); this.props.testResultDetail(row.data.value)}}>
...
(state) => ({
  ...
  typesDataDetail: state.pairwiseTypes.get('typesDataDetail'),
}),
(dispatch) => ({
  pairwiseTypesActions: bindActionCreators(pairwiseTypesActions, dispatch)
})
)(TypeFieldMappingTable);
