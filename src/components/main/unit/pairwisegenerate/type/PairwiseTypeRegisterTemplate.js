...
import PairwiseTypeRegisterTable from 'components/main/unit/pairwisegenerate/type/PairwiseTypeRegisterTable';
import PairwiseTypeRegisterInformationTemplate from 'components/main/unit/pairwisegenerate/type/PairwiseTypeRegisterInformationTemplate';
import ListCountNavigation from 'components/common/policy/ListCountNavigation';
import styles from'./PairwiseTypeRegisterTemplate.scss';
import classNames from 'classnames/bind';
const cx = classNames.bind(styles);

class PairwiseTypeRegisterTemplate extends Component {
  constructor(props) {
    super(props);
    console.log("ccccccccccccccccccccccc");
    this.state = {
      title: "조합 유형 목록 등록", /*테이블 명*/
      totalCnt: 0, /*총 데이터 수(row개수)*/
      isTotalCnt: 1, /*totalCnt를 조회 할지 말지*/
      pairwiseGenRuleList: [], /*테이블로 전달할 배열*/
  ...
  remoteViewTableList = (pageNum, pageCnt, methodNo, searchRuleName, searchRuleDescription, searchRuleLevel, searchGenerator, searchGenerateDateFrom, searchGenerateDateTo) => {
    console.log("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
    const url = '/pairwiseGeneration/viewPairwiseGenerationRuleList/1'; //나중에 executionUuid 수정
    const data = {
    ...
    Util.processAxios(url, data, (msg, response) =>{
      ...
      if (response == null){
        ...
        this.setState({
          pairwiseGenRuleList:  pairwiseGenRuleList,
          totalCnt: 0,
        ...
        pairwiseGenRuleList = response.data.body.pairwiseGenRuleList;
        pairwiseGenRuleList.map(( pairwiseGenRuleListMap, i) => {
          pairwiseGenRuleListMap = {...pairwiseGenRuleListMap,"rowType": "R","prevRowType":"R", "rowSeqNo": i};
          pairwiseGenRuleList[i] =  pairwiseGenRuleListMap;
          return true;
        })
        //setState 로 state에 다시 변경된 list 다시 저장
        this.setState({
          pairwiseGenRuleList: pairwiseGenRuleList,
          projectName: pairwiseGenRuleList[0].fileName,
          methodName: pairwiseGenRuleList[0].methodName,
  ...
  render() {
    ...
    return(
      <div className = 'pairwise-type-register-template'>
        <div className="pairwise-type-register-information-template" >
          <PairwiseTypeRegisterInformationTemplate {...this.state}
            fileName = {this.state.projectName}
            methodName = {this.state.methodName}
            pairwiseGenRuleList = {this.state.pairwiseGenRuleList}/>
        </div>
        {this.state.pairwiseGenRuleList != null && <PairwiseTypeRegisterTable {...this.state}
...
export default withRouter(PairwiseTypeRegisterTemplate);
