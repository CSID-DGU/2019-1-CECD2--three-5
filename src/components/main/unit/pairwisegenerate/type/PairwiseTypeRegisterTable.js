...
import PairwiseTypeFooter from 'components/main/unit/pairwisegenerate/type/PairwiseTypeFooter';
import PairwiseTypeRegisterToolBar from 'components/main/unit/pairwisegenerate/type/PairwiseTypeRegisterToolBar';
import styles from './PairwiseTypeRegisterTable.scss';
import classNames from 'classnames/bind';
import { random } from 'node-forge';
const cx = classNames.bind(styles);

class PairwiseTypeRegisterTable extends Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            selected: [],
            selectedList,
            pairwiseGenRuleList: this.props.pairwiseGenRuleList,
            show: false,
            searchRuleName : null,
    ...
    ****************************************/
    ...
    remoteBatchDelete(e){
        var pairwiseGenRuleList = this.state.pairwiseGenRuleList;
        var jsonArray =[];
        pairwiseGenRuleList = pairwiseGenRuleList.filter(function(item){
        ...
        if(pairwiseGenRuleList.length === 0){
      ...
      remoteBatchSave(e){
        ...
        if(pairwiseGenRuleList.length === 0){
          ...
          const data = { algorithmList: pairwiseGenRuleList };
    ...
    componentWillReceiveProps(nextProps) {
        this.setState({pairwiseGenRuleList: nextProps.pairwiseGenRuleList})
    }

    localAddRow(e){
        var seqNo;  //추가할 객체의 seqNo계산
        if(this.state.pairwiseGenRuleList.length <= 0) {
            seqNo = 1;
          } else {
            seqNo = this.state.pairwiseGenRuleList[this.state.pairwiseGenRuleList.length-1].rowSeqNo + 1;
        }
        const a = update(
            this.state.pairwiseGenRuleList,{
              $push: [{
                        ruleNo: seqNo,
                        ruleName: null,
                        rulDescription: null,
    ...
    *******************************************/
    ...
    render() {
        const pairwiseGenRuleList = this.state.pairwiseGenRuleList;
        ...
        return (
            <div className="pairwise-type-register-table">
                <ToolkitProvider keyField="executionUuid"
                    ...
                    {props => (
                        <div className ="pairwise-type-register-table">
                            ...
                            <PairwiseTypeRegisterToolBar {...this.state}
                                ...
                                className='pairwise-type-register-toolbar' />
                            <BootstrapTable
                                //{...props.baseProps}
                                //{...this.state}
                                keyField='ruleNo'
                                ...
                                afterSaveCell: (oldValue, newValue, row, column) =>{
                                  ...
                                  console.log(pairwiseGenRuleList[pairwiseGenRuleList.length-1]);
                                  if(oldValue !== newValue && seqNo < pairwiseGenRuleList[pairwiseGenRuleList.length-1].rowSeqNo && valid){
                                    console.log("이벤트 확인");
                                     this.localChangeRowType(seqNo);
                                  }
                                },
                                  nonEditableRows: function() { //rowType=="D, 해당 row는 editable:false"
                                  return pairwiseGenRuleList.filter(p => p.rowType=== "D").map(p => p.rowSeqNo);
                                  }
                            })}
                                />
                ...
                <div className={cx('footer')}>
                    <PairwiseTypeFooter {...this.state} totalCnt={this.props.totalCnt}
                        pageCnt={this.props.pageCnt}
                        pageNum={this.props.pageNum}
                        from={this.props.from}
                        to={this.props.to}
...
export default PairwiseTypeRegisterTable;
