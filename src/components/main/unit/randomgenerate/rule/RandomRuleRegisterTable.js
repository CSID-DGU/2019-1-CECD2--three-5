...
import styles from './RandomRuleRegisterTable.scss';
import classNames from 'classnames/bind';
import { random } from 'node-forge';
const cx = classNames.bind(styles);

class RandomRuleRegisterTable extends Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            selected: [],
            selectedList,
            randomGenRuleList: this.props.randomGenRuleList,
            show: false,
            searchIsUse: null,
            searchRuleName : null,
            searchRuleDescription : null,
            searchRuleLevel : null,
    ...
    * 선택해논 row executionUuid를 넣어놔 준다 (다건 삭제시 이용)*/
    handleOnSelect = (row, isSelect) => {
        if (isSelect) {
            ...
            row.rowType="D";
            this.setState(() => ({
                selected: [...this.state.selected, row.ruleNo]
            }));
            ...
            row.prevRowType="D";
            this.setState(() => ({
                selected: this.state.selected.filter(x => x !== row.ruleNo)
            }));
        }
    ...
    ****************************************/
    ...
    remoteBatchDelete(e){
        var randomGenRuleList = this.state.randomGenRuleList;
        var jsonArray =[];
        randomGenRuleList = randomGenRuleList.filter(function(item){
      ...
      remoteBatchSave(e){
        ...
        if(randomGenRuleList.length === 0){
          ...
          const data = { algorithmList: randomGenRuleList };
    ...
    componentWillReceiveProps(nextProps) {
        this.setState({randomGenRuleList: nextProps.randomGenRuleList})
    }

    localAddRow(e){
        var seqNo;  //추가할 객체의 seqNo계산
        if(this.state.randomGenRuleList.length <= 0) {
            seqNo = 1;
          } else {
            seqNo = this.state.randomGenRuleList[this.state.randomGenRuleList.length-1].rowSeqNo + 1;
        }
        const a = update(
            this.state.randomGenRuleList,{
              $push: [{
                        ruleNo: seqNo,
                        ruleName: null,
                        rulDescription: null,
    ...
    *********************************************/
    ...
    searchDetailCondition = (searchRuleName, searchRuleLevel, searchRuleDescription,searchGenerator, searchGenerateDateFrom, searchGenerateDateTo) => {
        const url = '/randomGeneration//viewRandomGenerationAlgorithmList/11111111';
        const data = {
            params: {
                searchRuleName: searchRuleName,
                searchRuleLevel: searchRuleLevel,
                searchRuleDescription: searchRuleDescription,
        ...
        Util.processAxios(url, data, (msg, response) => {
            this.props.remoteViewTableList(this.state.pageNum, this.state.pageCnt, searchRuleName, searchRuleLevel, searchRuleDescription,searchGenerator, searchGenerateDateFrom, searchGenerateDateTo);
        });
    }

    /* 3.2 검색조건 초기화 - 테이블 목록에서 선택한 검색 조건들을 Clear 버튼을 눌러 초기화 시켜준다.*/
    searchClear = () => {
        this.searchDetailCondition(null, null, null, null, null, null);
        this.setState({
            searchRuleName: null,
            searchRuleLevel: null,
            searchRuleDescription: null,
    ...
    *******************************************/
    ...
    formatWithIcon = (cell, row) => {
        ...
        return (
            <i className="fa fa-search-plus" idname="details" onClick={() => {
                this.props.testResults(row);
              }
            } />
        )
    }
    ...
    levelDropdown =(cell, row) =>{
        return(
            <div>
                <select defaultValue={cell} onChange={(e) => {row.ruleLevel = parseInt(e.target.value); this.localChangeRowType(row); }}>
    ...
    render() {
        const randomGenRuleList = this.state.randomGenRuleList;
        console.log("render props",this.props.randomGenRuleList);
        console.log("render state",this.state.randomGenRuleList);
                ...
                <ToolkitProvider keyField="executionUuid"
                    ...
                    {props => (
                        <div className ="random-rule-register-table">
                            ...
                            <RandomRuleRegisterToolBar {...this.state}
                                ...
                                className='random-rule-register-toolbar' />
                            <BootstrapTable
                                //{...props.baseProps}
                                //{...this.state}
                                keyField='ruleNo'
                                ...
                                afterSaveCell: (oldValue, newValue, row, column) =>{
                                  ...
                                  console.log(randomGenRuleList[randomGenRuleList.length-1]);
                                  if(oldValue !== newValue && seqNo < randomGenRuleList[randomGenRuleList.length-1].rowSeqNo && valid){
                                    console.log("이벤트 확인");
                                     this.localChangeRowType(seqNo);
                                  }
                                },
                                  nonEditableRows: function() { //rowType=="D, 해당 row는 editable:false"
                                  return randomGenRuleList.filter(p => p.rowType=== "D").map(p => p.rowSeqNo);
                                  }
                            })}
                                />
...
export default RandomRuleRegisterTable;
