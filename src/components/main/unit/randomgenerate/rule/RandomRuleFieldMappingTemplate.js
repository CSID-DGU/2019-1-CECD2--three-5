...
import RandomRuleFieldMappingInformationTemplate from 'components/main/unit/randomgenerate/rule/RandomRuleFieldMappingInformationTemplate';
...
import RuleFieldMappingContainer from 'components/main/unit/randomgenerate/treetables/rule/RuleFieldMappingContainer';
import styles from'./RandomRuleFieldMappingTemplate.scss';
...
class RandomRuleFieldMappingTemplate extends Component {
  constructor(props) {
    super(props);
    ...
    console.log("RandomRuleFieldMappingTemplate:constructor");
  }

  /* 정재웅 코드 부분 */
  initialize = async () => {
    const { RandomRulesActions } = this.props;
    try {
      //await RandomRulesActions.getRulesJSON(HARD_CODED_VALUE.TEMP_TESTCASE_NO);
      await RandomRulesActions.getRulesJSON(this.props.ruleNo);

      console.log("rulesData=>", this.props.rulesData);
    } catch (e) {
      console.log(e);
    }
  }
  ...
  componentDidMount(props) {
    console.log("RandomRuleFieldMappingTemplate:componentDidMount");
    this.initialize();
  ...
  render() {
    ...
    const{loading, rulesData} = this.props;
    if(loading || loading==undefined){
      console.log("no loading...");
      return null;
    }

    console.log("render::rulesData=>", rulesData);

    return(
      <div className = 'random-register-result-template'>
        <div>
          <RandomRuleFieldMappingInformationTemplate {...this.state}
        ...
        <br></br>
        {this.state.testcaseListInfo != null && <RuleFieldMappingContainer {...this.state}
...
(state) => ({
  ...
  loading: state.pender.pending['randomRules/GET_RULES_DATA']
}),
(dispatch) => ({
  RandomRulesActions: bindActionCreators(randomRulesActions, dispatch)
})
)(RandomRuleFieldMappingTemplate);
