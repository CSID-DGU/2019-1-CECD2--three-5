/**
 * 작성자: 김지운, 원예인
 * 설명: 컬럼 및 버튼을 정의하고 react-bootstrap-table을 이용하여 필드유형과 알고리즘을 표시한다.
**/
...
import RandomFieldFooter from 'components/main/unit/randomgenerate/field/RandomFieldFooter';
import RandomFieldToolBar from 'components/main/unit/randomgenerate/field/RandomFieldToolBar';
import styles from './RandomFieldMappingTable.scss';
import classNames from 'classnames/bind';
import { random } from 'node-forge';
const cx = classNames.bind(styles);
  
class RandomFieldMappingTable extends Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            selected: [],
            selectedList,
            randomGenFieldTypeList: this.props.randomGenFieldTypeList,
            show: false,
            searchIsUse: null,
            searchNum: null,
            searchMessage: null,
    ...
    * 선택해논 row executionUuid를 넣어놔 준다 (다건 삭제시 이용)*/
    handleOnSelect = (row, isSelect) => {
        if (isSelect) {
            ...
            row.rowType="D";
            this.setState(() => ({
                selected: [...this.state.selected, row.fieldTypeNo]
            }));
            ...
            row.prevRowType="D";
            this.setState(() => ({
                selected: this.state.selected.filter(x => x !== row.fieldTypeNo)
            }));
        }
    ...
    handleOnSelectAll = (isSelect, rows) => {
        const ids = rows.map(r => r.fieldTypeNo);
    ...
    ****************************************/
    ...
    remoteBatchDelete(e){
        var randomGenFieldTypeList = this.state.randomGenFieldTypeList;
        var jsonArray =[];
        randomGenFieldTypeList = randomGenFieldTypeList.filter(function(item){
          return  (item.rowType === "D");
        });
        console.log("삭제;;;;;;",this.state.selected);
        
        for(let i = 0; i < this.state.selected.length; i++){
            jsonArray[i] = {"fieldTypeNo": this.state.selected[i]};
        }

        if(randomGenFieldTypeList.length === 0){
          alert("삭제 할 레코드를 선택해 주시기 바랍니다.");
        }else{
          const url = '/randomGeneration/removeRandomGenerationFieldTypeList/11111111';
      ...
      remoteBatchSave(e){
        ...
        if(randomGenFieldTypeList.length === 0){
          ...
          const url = '/randomGeneration/saveRandomGenerationFieldTypeList/11111111';
          const data = { saveList: randomGenFieldTypeList };
    ...
    componentWillReceiveProps(nextProps) {
        this.setState({randomGenFieldTypeList: nextProps.randomGenFieldTypeList})
    }

    localAddRow(e){
        var seqNo;  //추가할 객체의 seqNo계산
        if(this.state.randomGenFieldTypeList.length <= 0) {
            seqNo = 1;
          } else {
            seqNo = this.state.randomGenFieldTypeList[this.state.randomGenFieldTypeList.length-1].rowSeqNo + 1;
        }
        const a = update(
            this.state.randomGenFieldTypeList,{
              $push: [{ 
                        level: 1,
                        fieldTypeName: null,
    ...
    *******************************************/
    ...
    columns = [
        {
            dataField: 'algorithmLevel',
            text: '레벨',
            editable: false,
            headerStyle: { width: '14%' },
            align: 'center',
            formatter:this.levelDropdown,
            style: {
                whiteSpace: 'nowrap',
        ...
        },
        {
            ...
            align: 'center',
            formatter:this.algorithmDropdown,
            style: {
                whiteSpace: 'nowrap',
                textOverflow: 'ellipsis',
                overflow: 'hidden'
    ...
    render() {
        const randomGenFieldTypeList = this.state.randomGenFieldTypeList;
        console.log("render props",this.props.randomGenFieldTypeList);
        console.log("render state",this.state.randomGenFieldTypeList);
        ...
        return (
            <div className="random-field-mapping-table">
                    ...
                    {props => (
                        <div className ="random-field-mapping-table">
                            ...
                            <RandomFieldToolBar {...this.state}
                                ...
                                className='random-field-toolbar' />
                            <BootstrapTable
                                //{...props.baseProps}
                                //{...this.state}
                                keyField='fieldTypeNo'
                                data={randomGenFieldTypeList}
                                ...
                                afterSaveCell: (oldValue, newValue, row, column) =>{
                                  ...
                                  console.log(randomGenFieldTypeList[randomGenFieldTypeList.length-1]);
                                  if(oldValue !== newValue && seqNo < randomGenFieldTypeList[randomGenFieldTypeList.length-1].rowSeqNo && valid){
                                    console.log("이벤트 확인");
                                     this.localChangeRowType(seqNo);
                                  }
                                },
                                  nonEditableRows: function() { //rowType=="D, 해당 row는 editable:false"
                                  return randomGenFieldTypeList.filter(p => p.rowType=== "D").map(p => p.rowSeqNo);
                                  }
                            })}
                                />
                ...
                <div className={cx('footer')}>
                    <RandomFieldFooter {...this.state} totalCnt={this.props.totalCnt}
                        pageCnt={this.props.pageCnt}
                        pageNum={this.props.pageNum}
                        from={this.props.from}
                        to={this.props.to}
...
export default RandomFieldMappingTable;