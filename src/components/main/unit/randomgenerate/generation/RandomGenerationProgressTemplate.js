...
class RandomGenerationProgressTemplate extends Component {
  ...
  executeSourceCollectionAxios (requestInfo, data, callback) {
    ...
    whaxios.post('/randomGeneration/executeRandomGeneration/'+this.uuid, requestInfo)
    .then(function(response){
      callback(ret);
    }).catch(function(error){
      console.log("catch error ====>");
...
export default RandomGenerationProgressTemplate;
